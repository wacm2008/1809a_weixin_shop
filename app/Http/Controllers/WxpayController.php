...
class WxpayController extends Controller
{
    ...
    public $notify_url = 'http://1809bilige.comcto.com/weixin/notify';     // 支付回调
    /**
     * 微信支付测试
     */
    public function pay()
    {
        $oid = intval($_GET['o_id']);    // 订单id
        $o_info = OrderModel::where(['o_id'=>$oid])->first();
        if(!$o_info){
            die("订单不存在");
        }
